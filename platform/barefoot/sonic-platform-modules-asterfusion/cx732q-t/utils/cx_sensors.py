...
def get_core_voltage():
    ...
    try:
        print("Barefoot VDDA 1.8V  : %.2f V\n" % (float(data.vrail1)/1000))
        print("Barefoot VDDA 1.7V  : %.2f V\n" % (float(data.vrail2)/1000))
        print("Payload 12V         : %.2f V\n" % (float(data.vrail3)/1000))
        print("Payload 3.3V        : %.2f V\n" % (float(data.vrail4)/1000))
        print("Payload 5V          : %.2f V\n" % (float(data.vrail5)/1000))
        print("Barefoot VDD 1.8V   : %.2f V\n" % (float(data.vrail6)/1000))
        print("Barefoot Core       : %.2f V\n" % (float(data.vrail7)/1000))
        print("Barefoot VDDT 0.86V : %.2f V\n" % (float(data.vrail8)/1000))
    except:
        pass
...
def get_fan_info():
    ...
    try:
        ...
        print("FAN 6F:")
        print("    Status:   Normal" if fan_data[10].front_rpm > 0 else \
              "    Status:   No Fan")
        print("    Speed:    %d r/min\n" % fan_data[10].front_rpm)

        print("FAN 6R:")
        print("    Status:   Normal" if fan_data[11].front_rpm > 0 else \
              "    Status:   No Fan")
        print("    Speed:    %d r/min\n" % fan_data[11].front_rpm)
    except:
        pass
