...
function generate_platform_config() {
    ...
    if [ $enable_uart -eq 0 ] && [ "$xt_platform"X = ""X ]; then
        ...
        if [[ $rc =~ "312" ]]; then
            ...
            i2cset -y "$default_i2c" 0x3e 0x11 0xaa 0xaa s
            IFS=' ' read -r -a bmcver_arr <<< $(sudo i2cdump -y 2 0x3e s 0xff | tail -n1)
            bmc_version="${bmcver_arr[@]:2:${bmcver_arr[1]}}"
        fi
    fi
...
function get_bmc_mac_address() {
    ...
    if [ ! -e $mac_cache ]; then
        ...
        i2c=`i2cdetect -l | awk -F '[- ]' '/CP2112/{print $2}'`
        cp2112=${i2c:0:1}
        sudo i2cset -y $superio 0x3e 0x01 0x24 0xaa s
        val=`sudo i2cdump -y $superio 0x3e s 0xff 2>/dev/null`
        if [ $? -ne 0 ]; then
            sudo i2cset -y $cp2112 0x3e 0x01 0x24 0xaa s
            val=`sudo i2cdump -y $cp2112 0x3e s 0xff 2>/dev/null`
            if [ $? -ne 0 ]; then
                echo "Error: Failed to get system mac address."
            fi
        fi
        if [ $? -eq 0 ]; then
            mac=${val:79:17}
