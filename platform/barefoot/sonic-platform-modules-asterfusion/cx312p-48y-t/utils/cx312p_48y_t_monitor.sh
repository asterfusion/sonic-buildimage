...
function setup_hardware() {
    ...
    sudo ifconfig eth0:1 $(cat $device/$platform/GHC0_ip)
    sudo ifconfig eth0:2 $(cat $device/$platform/GHC1_ip)
...
case "$1" in
    ...
    restart)
        ...
        ;;
    status)
        echo "Asterfusion CX312P-48Y-T is running..."
        ;;
    *)
