...
class Sff8472Api(XcvrApi):
    ...
    def get_transceiver_info(self):
        ...
        if ext_spec_compliance:
            spec_compliance[consts.EXT_SPEC_COMPLIANCE_FIELD] = ext_spec_compliance
        if not self.xcvr_eeprom.read(consts.EXT_CAL_FIELD):
            second_ext_spec_compliance = self.xcvr_eeprom.read(consts.SECOND_EXT_SPEC_COMPLIANCE_FIELD)
            if second_ext_spec_compliance:
                spec_compliance[consts.SECOND_EXT_SPEC_COMPLIANCE_FIELD] = second_ext_spec_compliance
