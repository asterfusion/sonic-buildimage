...
class CmisApi(XcvrApi):
    ...
    def get_transceiver_info(self):
        ...
        # Asterfusion fix: beautify application advertisement
        xcvr_info['application_advertisement'] = self.get_beautified_application_advertisement()
        # Asterfusion fix end
    ...
    def get_laser_TEC_current(self):
        ...
        if aux1_mon_type == 1:
            # Asterfusion fix: occasionally TypeError when transceiver is removed
            laser_tec_current = self.xcvr_eeprom.read(consts.AUX1_MON)
            laser_tec_current_high_alarm = self.xcvr_eeprom.read(consts.AUX1_HIGH_ALARM)
            laser_tec_current_low_alarm = self.xcvr_eeprom.read(consts.AUX1_LOW_ALARM)
            laser_tec_current_high_warn = self.xcvr_eeprom.read(consts.AUX1_HIGH_WARN)
            laser_tec_current_low_warn = self.xcvr_eeprom.read(consts.AUX1_LOW_WARN)
            read_failed = laser_tec_current is None or \
                          laser_tec_current_high_alarm is None or \
                          laser_tec_current_low_alarm is None or \
                          laser_tec_current_high_warn is None or \
                          laser_tec_current_low_warn is None
            if read_failed:
                return None
            laser_tec_current /= LASER_TEC_CURRENT_SCALE
            laser_tec_current_high_alarm /= LASER_TEC_CURRENT_SCALE
            laser_tec_current_low_alarm /= LASER_TEC_CURRENT_SCALE
            laser_tec_current_high_warn /= LASER_TEC_CURRENT_SCALE
            laser_tec_current_low_warn /= LASER_TEC_CURRENT_SCALE
            # Asterfusion fix end
        elif aux1_mon_type == 0 and aux2_mon_type == 1:
            # Asterfusion fix: occasionally TypeError when transceiver is removed
            laser_tec_current = self.xcvr_eeprom.read(consts.AUX2_MON)
            laser_tec_current_high_alarm = self.xcvr_eeprom.read(consts.AUX2_HIGH_ALARM)
            laser_tec_current_low_alarm = self.xcvr_eeprom.read(consts.AUX2_LOW_ALARM)
            laser_tec_current_high_warn = self.xcvr_eeprom.read(consts.AUX2_HIGH_WARN)
            laser_tec_current_low_warn = self.xcvr_eeprom.read(consts.AUX2_LOW_WARN)
            read_failed = laser_tec_current is None or \
                          laser_tec_current_high_alarm is None or \
                          laser_tec_current_low_alarm is None or \
                          laser_tec_current_high_warn is None or \
    ...
    # Asterfusion fix: beautify application advertisement
    def get_beautified_application_advertisement(self):
        """
        Get the application advertisement of the CMIS transceiver

        Returns:
            String, the application advertisement string in beautified format
        """
        map = {
            Sff8024.MODULE_MEDIA_TYPE[1]: consts.MODULE_MEDIA_INTERFACE_850NM,
            Sff8024.MODULE_MEDIA_TYPE[2]: consts.MODULE_MEDIA_INTERFACE_SM,
        ...
        for app in range(1, 16):
            ...
            if val in [None, 'Unknown', 'Undefined']:
                break
            host_buf.append('Host Electrical Interface Id: {}'.format(val))

            key = "{}_{}".format(consts.HOST_LANE_ASSIGNMENT_OPTION, app)
            val = dic.get(key)
            if val is None:
                break
            host_buf.append('Host Lane Assignment Options: {}'.format(val))

            key = "{}_{}".format(consts.HOST_LANE_COUNT, app)
            val = dic.get(key)
            if val is None:
                break
            host_buf.append('Host Lane Count: {}'.format(val))
            ...
            if val in [None, 'Unknown', 'Undefined']:
                break
            media_buf.append('Module Media Interface Id: {}'.format(val))

            key = "{}_{}".format(consts.MEDIA_LANE_ASSIGNMENT_OPTION, app)
            val = dic.get(key)
            if val is None:
                break
            media_buf.append('Media Lane Assignment Options: {}'.format(val))

            key = "{}_{}".format(consts.MEDIA_LANE_COUNT, app)
            val = dic.get(key)
            if val is None:
                break
            media_buf.append('Media Lane Count: {}'.format(val))

            ret.append("\t\tApplication Selection 0x{:x}:\n\t\t\t{}\n\t\t\t{}".format(
                app, ", ".join(host_buf), ", ".join(media_buf))
            )
        return '\n{}'.format('\n'.join(ret))
    # Asterfusion fix end
